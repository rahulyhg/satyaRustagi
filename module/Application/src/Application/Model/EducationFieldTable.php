...
class EducationFieldTable extends AbstractTableGateway {

    protected $table = 'tbl_education_field';
...
//            $data['created'] = date("Y-m-d H:i:s");
    ...
    public function selectList($columns=array('id,gothra_name')){
        $gothra= $this->select(function (Select $select) use ($columns) {
            $select->order('id ASC');
            $select->columns($columns);
            
        })->toArray();
        foreach($gothra as $c)$list[$c[$columns[0]]]=$c[$columns[1]];
        return $list;
    }
