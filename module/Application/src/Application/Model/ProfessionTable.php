...
class ProfessionTable extends AbstractTableGateway {
...
//            $data['created'] = date("Y-m-d H:i:s");
    ...
    public function selectList($columns=array('id,profession')){
        $profession= $this->select(function (Select $select) use ($columns) {
            $select->order('id ASC');
            $select->columns($columns);
            
        })->toArray();
        foreach($profession as $c)$list[$c[$columns[0]]]=$c[$columns[1]];
        return $list;
    }
